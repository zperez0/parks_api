...
describe "edit a park route", type: :request do
  before do
    ...
    patch "/api/v1/parks/#{@park.id}", params: { name: "new name", longitude: 567, latitude: 612, statecode: "WA", animal: "new_animal", kind: "national", description: "new_description" }
  end
...
describe "exceptions work as expected", type: :request do
  it "throws error if name longitude, latitude, statecode, animal,kind, description field is empty" do
    ...
    patch "/api/v1/parks/#{@park.id}", params: { name: "", statecode: "", animal: "", kind: "", description: "" }
